...
function TourProviderInner({
	...
	const currentStep = onboardState?.currentStep
		? stepsData.find((s) => s.id === (onboardState.currentStep as any).id) ||
			null
		: null;
	...
	const currentStepIndex = currentStepId
		? stepIds.indexOf(String(currentStepId))
		: 0;
...
function StepsBuilder({ children }: { children: React.ReactNode }) {
	...
	const { data: googleSession, isLoading: googleLoading } =
	...
	const stepsData: TourStepData[] = useMemo(() => {
		...
		s.push({
			...
			description: "Now let's add some YouTube channels to your group!",
	...
	return (
		...
		>
			<TourProviderInner stepsData={stepsData}>{children}</TourProviderInner>
		</OnboardingProvider>
	);
}
...
export function useOnboardingTour(): TourContextValue {
	...
	if (!ctx)
		throw new Error("useOnboardingTour must be used within a TourProvider");
	return ctx;
}
