	...
	return (
		<div className="space-y-4">
				...
				<div className="flex items-center justify-between p-3 rounded-lg border">
					<div className="flex items-center gap-3">
						...
						<div>
							...
							{socialConnections.google.connected ? <Badge variant="secondary" className="mt-1 gap-1 h-auto py-0.5 px-2"><Shield className="h-3 w-3" /> {t("account.connected")}</Badge> : <span className="text-xs text-muted-foreground">{t("account.notconnected")}</span>}
						</div>
					</div>
					<Button size="sm" variant={socialConnections.google.connected ? "outline" : "ghost"} data-tour="google-connect-btn" onClick={() => socialConnections.google.connected ? handleDisconnect("google") : handleConnect("google")}>
						{socialConnections.google.connected ? <><Unlink className="h-3.5 w-3.5 mr-1" /> {t("account.disconnect")}</> : <><Link className="h-3.5 w-3.5 mr-1" /> {t("account.connect")}</>}
					</Button>
				</div>
				...
				<div className="flex items-center justify-between p-3 rounded-lg border">
					<div className="flex items-center gap-3">
						...
						<div>
							...
							{socialConnections.discord.connected ? <Badge variant="secondary" className="mt-1 gap-1 h-auto py-0.5 px-2"><Shield className="h-3 w-3" /> {t("account.connected")}</Badge> : <span className="text-xs text-muted-foreground">{t("account.notconnected")}</span>}
						</div>
					</div>
					<Button size="sm" variant={socialConnections.discord.connected ? "outline" : "ghost"} onClick={() => socialConnections.discord.connected ? handleDisconnect("discord") : handleConnect("discord")}>
						{socialConnections.discord.connected ? <><Unlink className="h-3.5 w-3.5 mr-1" /> {t("account.disconnect")}</> : <><Link className="h-3.5 w-3.5 mr-1" /> {t("account.connect")}</>}
					</Button>
				</div>
			</div>
			...
			<div className="rounded-xl border bg-card/50 backdrop-blur-sm p-4">
				<h2 className="font-semibold text-sm mb-3">{t("account.password.section")}</h2>
				<div className="space-y-3">
					<Input type={showNewPassword ? "text" : "password"} placeholder={t("account.newpassword")} value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="pr-10 h-9" />
					...
					<Input type={showConfirmPassword ? "text" : "password"} placeholder={t("account.confirmpassword")} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="pr-10 h-9" />
					
					<Button variant="secondary" size="sm" disabled={!newPassword || newPassword !== confirmPassword || newPassword.length < 6 || passwordMutation} onClick={() => updatePasswordMutation({ password: newPassword, passwordConfirmation: confirmPassword })}>{t("account.updatepassword")}</Button>
				</div>
			</div>
			...
			<div className="rounded-xl border bg-card/50 backdrop-blur-sm p-4">
				...
				<div className="flex items-center justify-between p-3 rounded-lg border">
					<div className="flex items-center gap-3">
						...
						<div className="flex flex-col">
							...
							<span className="text-xs text-muted-foreground">{t("account.retour.desc")}</span>
						</div>
					</div>
					<Button size="sm" variant="outline" onClick={handleRedoTour}>
						<RefreshCw className="h-3.5 w-3.5 mr-1" /> {t("account.retour.button")}
					</Button>
				</div>
				<h2 className="font-semibold text-sm mb-3 mt-6">{t("account.danger")}</h2>
				<Dialog open={deleteDialogOpen} onOpenChange={(o) => { setDeleteDialogOpen(o); if (!o) { setDeleteConfirmation(""); setAcknowledgeDataLoss(false); setAcknowledgeNoRecovery(false); } }}>
					<DialogTrigger asChild><Button variant="destructive" size="sm"><Trash2 className="h-3.5 w-3.5 mr-1" /> {t("account.delete")}</Button></DialogTrigger>
					<DialogContent>
						<DialogHeader>
							<DialogTitle className="flex items-center gap-2 text-red-600"><AlertTriangle className="h-5 w-5" /> {t("account.delete.confirm")}</DialogTitle>
							<DialogDescription>{t("account.delete.irreversible")}</DialogDescription>
						</DialogHeader>
						<div className="space-y-4 mt-4">
							<p className="text-sm text-muted-foreground">{t("account.delete.body")}</p>
							<Checkbox checked={acknowledgeDataLoss} onCheckedChange={(c) => setAcknowledgeDataLoss(c as boolean)} /><Label>{t("account.delete.check1")}</Label>
							<Checkbox checked={acknowledgeNoRecovery} onCheckedChange={(c) => setAcknowledgeNoRecovery(c as boolean)} /><Label>{t("account.delete.check2")}</Label>
							<Input placeholder={t("account.delete.type")} value={deleteConfirmation} onChange={(e) => setDeleteConfirmation(e.target.value)} />
						</div>
						<DialogFooter className="gap-2 mt-4">
							<Button variant="outline" size="sm" onClick={() => setDeleteDialogOpen(false)}>{t("account.delete.cancel")}</Button>
							<Button variant="destructive" size="sm" disabled={!canDeleteAccount} onClick={() => deleteAccountMutation.mutateAsync()}>{t("account.delete.confirm.btn")}</Button>
						</DialogFooter>
					</DialogContent>
				</Dialog>
			</div>
